%**************************************************************************
%*******FORWARD KINEMATICS (3D) of Any DOF's ROBOT USING DISTAL DH PARAMETERS *
%**************************************************************************
% WHERE
% NJ = Number of Joints 
% NF = Number of Frames
...
f2 = fopen(inputFileDHP,'r')
input_variables = fscanf(f2,'%d',5)
NJ = input_variables(1);
starting_frame = input_variables(4);
ending_frame = input_variables(5);
...
[A,flag] = ArmMatrix_distal(A,inputFileDHP);

% Calculate the tip to base tip to base concatenated matrix
% tip_frame = 9; % frame at Ankle tip
% base_frame = 0; % frame at base or pelvis

A1 = T_Concat_distal(A,ending_frame,starting_frame); % Here  
